...
class ScoreManager {
    constructor() {
      ...
      this.config = {}
      this.timer = null;
      this.add_vel = 1; // scores to be added per second
      this.step = 100;
    ...
    flash() {
      ...
      if(this.score >= 200 && this.lvl == 0) {
        // inc lvl
        this.lvl = 1;
        enemy.spawnPteros();
        logs.log('Pterodactyls started to spawn');
      } else if(this.score >= 500 && this.lvl == 1) {
        // inc lvl
        this.lvl = 2;
        this.add_vel = 20; //twice the score gain speed
        logs.log('Score level 2');
      } else if(this.score >= 1000 && this.lvl == 2) {
        // inc lvl
        this.lvl = 3;
        this.add_vel = 40; //twice the score gain speed
        logs.log('Score level 3');
      } else if (this.score >=1500 && this.lvl==3){
        this.lvl = 4;
        this.add_vel = 60; //twice the score gain speed
        logs.log('Level:Runner');
      }else if (this.score >=1800 && this.lvl == 4){
        this.lvl = 5;
        this.add_vel = 80; //twice the score gain speed
        logs.log('Level:Hot Chicken');
      }else if (this.score >=2000 && this.lvl == 5){
        this.lvl = 6;
        this.add_vel = 100;//twice the score gain speed
        this.log('Level:Monster')
      }
    }
