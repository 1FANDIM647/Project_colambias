...
prx.types.vectoranimation = {
    ...
    , onDisplay: function (item, containerid, symbol) {
        ...
        if (prx.editor && prx.componentsHelper.getProp(item.imgSrc.fileId,'other') == '5222c5ba6594f582d406726697aebb2f.json' && prx.componentsHelper.getProp(item.imgSrc.assetType,'other') == 'gallery') {
            cR += '<div class="type-vectoranimation-hover-message">Double-click to edit vector</div>';
...
prx.types.basic_tabbar = {
    ...
    ,onDisplay: function(item,containerid,symbol) {
        ...
        if(item.maskEnabled && $.browser.webkit) {
            cR += '#'+_id+' .icon { '+prx.gradients.getBgCssByProperty(item.maskInactive,'color-background')+'; -webkit-mask-size: auto '+(prx.componentsHelper.getProp(item.iconSize,'num-other') * 20)+'%; }';
        ...
        $.each(item.tabs, function(i,elm){
            ...
            if(prx.componentsHelper.getProp(item.iconpos,'other') != '') {
                ...
                cR += '</style>';
                cR += '<div class="icon-wrapper"><div class="icon '+ ((prx.componentsHelper.getProp(item.maskEnabled,'boolean') && $.browser.webkit) ? ((prx.componentsHelper.getProp(item.activeTab,'num-other') == i) ? 'liveUpdate-maskActive-background-color' : 'liveUpdate-maskInactive-background-color') : '') +'"></div>';
            }
            if(item.iconpos != 'notext') {
